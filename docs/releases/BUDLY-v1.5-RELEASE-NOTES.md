...
Administrators receive capability- and nonce-protected search, filters, correction, invalidation, deletion, export, expiration review, health metrics, and audit inspection. Recommendations can receive a server-built deterministic context; decision evidence records only governed memory references and continues to enforce catalog, no-match, sensitive-domain, and escalation controls.

Schema 1.3.0 is additive and rollback retains the new tables dormant to prevent data loss. Production deployment and Gate F remain blocked.
