...
document.querySelectorAll('[data-budly-sales]').forEach(root=>{
 ...
 const conversationInput=()=>input('<input name="answer" aria-label="Message Budly" placeholder="Ask Budly anything within the approved knowledge available" maxlength="4000" required>','Send');
 ...
 const activateLegacy=message=>{state.goal=message;const [,j]=route(message);state.journey=j;state.answers=[];state.q=0;state.step='questions';bubble('I’ll continue with the guided '+j.label+' path.','bot','Guided Budly');ask()};
 const conversationalTurn=async message=>{const r=await fetch(cfg.conversationUrl,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-WP-Nonce':cfg.conversationNonce},body:JSON.stringify({conversation_id:state.session,message})});const out=await r.json();if(!r.ok||!out.success)throw new Error(out?.error?.message||'Conversational Budly is temporarily unavailable.');const response=out.data.response;bubble(response.text,'bot','Budly');if(response.requires_human||response.resulting_action==='human_handoff'){offerHumanHelp();state.step='done';return}if(response.resulting_action==='legacy_guided_flow'){activateLegacy(message);return}if(response.selected_product_id){const products=await loadProducts(),p=products.find(item=>item.slug===response.selected_product_id);if(p)card(p)}state.step='conversation';conversationInput()};
 ...
 form.addEventListener('click',e=>{const returning=e.target.closest('[data-budly-returning]');if(returning){root.dataset.budlyConversationId=state.session;window.BudlySecureMemory.open(root,profile=>{state.name=profile?.preferred_name||'there';state.verified=true;state.memoryConsent=Boolean(profile?.memoryApproved);state.decisionCsrf=profile?.budlyCsrf||'';track('identity_verified');state.step=cfg.conversationEnabled?'conversation':'goal';bubble('What can I help you with today?');if(cfg.conversationEnabled)conversationInput();else goalInput()});return}const starter=e.target.closest('[data-budly-starter]');if(!starter)return;track('starter_selected',{metadata:starter.textContent.trim()});const field=fields.querySelector('input[name="answer"]');if(field){field.value=starter.dataset.budlyStarter;form.requestSubmit()}});
 form.addEventListener('submit',async e=>{
   ...
   try{
     ...
     else if(state.step==='recall-code'){const result=await recallVerify(state.email,d.recall_code);state.name=result.profile.name||'there';state.email=result.profile.email;state.phone=result.profile.phone||'';state.memoryConsent=true;state.verified=true;track('identity_verified');if(result.history?.length)track('history_recalled',{metadata:String(result.history.length)});bubble(result.message,'bot','Email verified');showRecallHistory(result.history);state.step=cfg.conversationEnabled?'conversation':'goal';bubble('What can I help you with today?');if(cfg.conversationEnabled)conversationInput();else goalInput()}
     else if(state.step==='identity'){state.name=d.name;state.email=d.email;state.phone=d.phone||'';state.memoryConsent=Boolean(d.memory_consent);track('conversation_started');bubble(d.name,'user');restart.hidden=false;if(cfg.conversationEnabled){state.step='conversation';bubble('Nice to meet you, '+d.name+'. What would you like to explore?');if(state.preselectedGoal){const first=state.preselectedGoal;bubble(first,'user');await conversationalTurn(first)}else conversationInput()}else if(state.preselectedGoal){state.goal=state.preselectedGoal;bubble(state.goal,'user');const [,j]=route(state.goal);state.journey=j;track('journey_selected');state.step='questions';bubble('Nice to meet you, '+d.name+'. I’ll use the '+j.label+' path so the next questions stay relevant.','bot',j.label);ask()}else{state.step='goal';bubble('Nice to meet you, '+d.name+'. What can I help you with today?');goalInput()}}
     else if(state.step==='conversation'){bubble(d.answer,'user');await conversationalTurn(d.answer)}
     else if(state.step==='goal'){state.goal=d.answer;bubble(d.answer,'user');if(hasRisk(d.answer)){state.journey=journeys.wellness;state.answers=['safety review','human assistance','no automated recommendation'];track('journey_selected');await finish();return}const [,j]=route(d.answer);state.journey=j;track('journey_selected');if(j===journeys.membership&&membershipPolicyRequested(d.answer))explainMembershipPolicy();state.step='questions';bubble('I’ll use the '+j.label+' path so the next questions stay relevant.','bot',j.label);ask()}
     else if(state.step==='questions'){state.answers.push(d.answer);bubble(d.answer,'user');state.q++;ask()}
   }catch(err){if(state.step==='conversation'&&d.answer){bubble('Conversational Budly is temporarily unavailable, so I’m switching to the guided experience.','bot','Safe fallback');activateLegacy(d.answer)}else bubble(err.message||'Something went wrong. Please try again.','bot','Could not verify')}finally{send.disabled=false}
 });
 root.querySelectorAll('[data-budly-hero-starter]').forEach(button=>button.addEventListener('click',()=>{root.querySelectorAll('[data-budly-hero-starter]').forEach(b=>b.classList.remove('is-selected'));button.classList.add('is-selected');state.preselectedGoal=button.dataset.budlyHeroStarter||'';track('starter_selected',{metadata:button.textContent.trim()});fields.querySelector('input[name="name"]')?.focus()}));
