...
final class WooCommerceAdapter{
 ...
 public function ingest($order_id,$event_type){
  ...
  $is_revenue=in_array($event_type,array('payment_completed','order_completed'),true)&&!$this->repo->recorded_revenue_event($order->get_id());
  ...
  $net=self::money((float)$gross-(float)$refund);
