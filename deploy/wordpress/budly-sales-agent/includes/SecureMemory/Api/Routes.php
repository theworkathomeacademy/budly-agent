...
final class Routes {
    ...
    public static function discovery(\WP_REST_Request $request){$goal=sanitize_text_field((string)($request->get_param('shopping_goal')?:''));$journey=sanitize_text_field((string)($request->get_param('journey')?:'general'));$confidence=max(0.0,min(1.0,(float)($request->get_param('confidence')?:0.5)));$known=array('shopping_goal'=>$goal);foreach(array('experience_level','preferred_format','budget_range','purchase_timeline') as $attribute){$value=sanitize_text_field((string)($request->get_param($attribute)?:''));if($value!=='')$known[$attribute]=$value;}$question=\Budly\Conversation\ConversationManager::select_next_adaptive_question($known,$journey,$confidence);return Response::success(array('next_question'=>$question,'known_attributes'=>$known));}
    public static function conversation_state(\WP_REST_Request $request){$customer=SessionGuard::verified_customer_id($request);if($customer instanceof \WP_REST_Response)return $customer;$conv_id=sanitize_text_field((string)($request->get_param('conversation_id')?:'conv-default'));return Response::success(\Budly\Conversation\ConversationManager::get_state($customer,$conv_id));}
    public static function update_conversation_state(\WP_REST_Request $request){$customer=SessionGuard::verified_customer_id($request,true);if($customer instanceof \WP_REST_Response)return $customer;$conv_id=sanitize_text_field((string)($request->get_param('conversation_id')?:'conv-default'));$new_state=sanitize_text_field((string)($request->get_param('state')?:'exploring'));$reason=sanitize_text_field((string)($request->get_param('reason')?:'user_update'));return Response::success(\Budly\Conversation\ConversationManager::transition_state($customer,$conv_id,$new_state,$reason));}
    public static function relationship(\WP_REST_Request $request){$customer=SessionGuard::verified_customer_id($request);if($customer instanceof \WP_REST_Response)return $customer;return Response::success(\Budly\Lifecycle\LifecycleEngine::get_relationship_health($customer));}
    public static function journey(\WP_REST_Request $request){$customer=SessionGuard::verified_customer_id($request);if($customer instanceof \WP_REST_Response)return $customer;$health=\Budly\Lifecycle\LifecycleEngine::get_relationship_health($customer);return Response::success(array('customer_id'=>$customer,'lifecycle_stage'=>$health['lifecycle_stage'],'milestones'=>$health['milestones']));}
    ...
    public static function admin_conversation_health(\WP_REST_Request $request){if(!self::admin_permission())return Response::error(Errors::ADMIN_PERMISSION_REQUIRED,'Administrator permission is required.',403);return Response::success(\Budly\Conversation\ConversationManager::health_metrics());}
    public static function admin_lifecycle(\WP_REST_Request $request){if(!self::admin_permission())return Response::error(Errors::ADMIN_PERMISSION_REQUIRED,'Administrator permission is required.',403);return Response::success(array('stages'=>\Budly\Lifecycle\LifecycleEngine::canonical_stages(),'distribution'=>\Budly\Lifecycle\LifecycleEngine::stage_distribution()));}
}
