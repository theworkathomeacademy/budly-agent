    ...
    def assemble(self, *, personality: dict[str, Any], session: Any, message: str, knowledge: list[dict[str, Any]]) -> dict[str, Any]:
        layers = [
            ...
            (PROMPT_ORDER[5], {
                ...
                "intents": session.current_intents,
                "customer_name": getattr(session, "customer_name", None),
                "customer_email": getattr(session, "customer_email", None),
                "name_capture_asked": getattr(session, "name_capture_asked", False),
                "name_capture_declined": getattr(session, "name_capture_declined", False),
                "email_capture_asked": getattr(session, "email_capture_asked", False),
                "email_capture_declined": getattr(session, "email_capture_declined", False),
                "crm_context_consent": getattr(session, "crm_context_consent", "none"),
                "follow_up_permission": getattr(session, "follow_up_permission", "none"),
            }),
